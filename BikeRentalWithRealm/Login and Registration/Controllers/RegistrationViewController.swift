...
class RegistrationViewController: UIViewController, UINavigationControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        registerButton.frame = CGRect(x: (view.bounds.width / 2) - (frameWidth / 2),y: view.bottom - 300, width: frameWidth, height: 52)
        
        label.frame = CGRect(x: (view.bounds.width / 2) - (frameWidth / 2),y: registerButton.bottom + 50, width: frameWidth, height: 52)
        
        loginButton.frame = CGRect(x: (view.bounds.width / 2) - (frameWidth / 2),y: label.bottom + 10, width: frameWidth, height: 52)
    ...
    @objc private func registerButtonTapped() {
        ...
        DatabaseManager.shared.userExists(with: email, completion: {[weak self] exists in
            ...
            guard !exists else {
                // user already exists
                strongSelf.alertUserLoginError(message: "Looks like user with the same email is already exists")
                return
            }
...
extension RegistrationViewController: UIImagePickerControllerDelegate {
    ...
    func presentPhotoActionSheet() {
        let actionSheet = UIAlertController(title: "Profile Picture", message: "How would you like to select a picture?", preferredStyle: .alert)
